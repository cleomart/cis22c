...
class SibTreeNode extends TreeNode {
	...
	private static final SibTreeNode SibTreeNode = null;
	/**
	...
	public void insertChild(Object item, int c) throws InvalidNodeException {
		...
		if (myTree != null) {
			...
			child.parent = this;
			SibTreeNode traverse = firstChild;

			if (c <= 1 || firstChild == null) {
				firstChild = child;
				child.nextSibling = traverse;
			}
			else {
				SibTreeNode next = firstChild.nextSibling;
				for( int i = 2; i < c && next != null; i++) {
					traverse = traverse.nextSibling;
					next = next.nextSibling;
					
				}
				traverse.nextSibling = child;
	...
	public void removeLeaf() throws InvalidNodeException {
		...
		if(this.isValidNode()){
			if (children() == 0) { //if the node is leaf
				if (this == myTree.root) {
					myTree.root = null;
				}
				else {
					SibTreeNode isMyParent = parent; // find myParent
					if (isMyParent.firstChild == this) {
						// am I myParent's first child?
						isMyParent.firstChild = this.nextSibling;
					}
					else {
						SibTreeNode prevSib = isMyParent.firstChild;
						while (prevSib.nextSibling != this) {
							// loop until finding myPrevSib
							prevSib = prevSib.nextSibling;
						}
						prevSib.nextSibling = nextSibling;
						// set myPrevSib's nextSibling to myNextSib
					}
				}
				valid = false;
				myTree.size--;
